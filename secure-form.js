/* ==========================================================================
   ...
   requestTimeout ms to wait for a confirmed save (default 45000)
...
(function (global) {
  ...
  SecureFormInstance.prototype._submit = function (e) {
    ...
    var timer = setTimeout(function () {
      controller.abort();
    }, this.opts.requestTimeout || 45000);

    Promise.resolve().then(function () {
      return fetch(self.opts.endpoint, {
        method: "POST",
        body: new URLSearchParams(new FormData(self.form)),
      ...
      if (!result || (result.result !== "success" && result.result !== "duplicate")) {
        throw new Error(result && result.message || "Order save was not confirmed.");
      }
      clearTimeout(timer);
      self._loader(false);
      try {
        if (typeof self.opts.onSuccess === "function") self.opts.onSuccess(result);
      } finally {
